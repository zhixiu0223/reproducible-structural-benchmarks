# [Re] Efficient geometric nonlinear elastic analysis for design of steel structures: Benchmark studies (Frame 1)

Status: draft. Target venue: ReScience C.

Original article: C.W. Ziemian, R.D. Ziemian (2021). Steel benchmark frames for structural analysis and validation studies: Finite element models and numerical simulation data. Data in Brief, 39, 107564.

Related article: C.W. Ziemian, R.D. Ziemian (2021). Efficient geometric nonlinear elastic analysis for design of steel structures: Benchmark studies. Journal of Constructional Steel Research, 186, 106870.

Data repository: https://doi.org/10.17632/39sjhchwtx.1 (Mendeley Data, CC BY 4.0)

## Abstract

We independently reproduced the elastic second-order (P-Delta) benchmark results for Frame 1 (single-story steel portal frame) from Ziemian and Ziemian (2021) and identified one undocumented methodological requirement -- the AISC Direct Analysis Method's 0.8EI stiffness reduction -- that is essential for successful replication and is not stated in the released data article's frame descriptions. Modeling the frame at the nominal, unreduced stiffness produces self-consistent but systematically low results (roughly 20-25% low relative to the published benchmark) that could easily be mistaken for a modeling error; the released FEA data even contains a same-named but unreduced "DAM" material entry that a reproducer could plausibly select in error. Once the correct reduction is identified and applied, cross-validation across four independent open-source finite element implementations (FRAME3DD, OpenSeesPy, suanPan, CalculiX) confirms the benchmark is reproducible within engineering tolerance (0.03-2.5% for the second-order comparison across three tools; the fourth, CalculiX, is compared against the first-order benchmark only, for reasons discussed below). The same 0.8E requirement was independently confirmed on two further, structurally dissimilar benchmark frames from the same dataset: a highly sensitive multi-bay leaning-column frame (alpha_cr = 1.19) and an asymmetric two-story frame (alpha_cr = 2.62); for the latter we additionally tested and ruled out the nominal (unreduced) and GMNIA-associated 0.9E stiffness values also present in the released material tables, leaving 0.8E as the only value matching the published benchmark.

## Introduction

Ziemian and Ziemian (2021, Data in Brief) published the complete geometry, member sizes, loading, and simulation results for 22 planar steel benchmark frames, released under CC BY 4.0 on Mendeley Data. The companion methods paper (Ziemian and Ziemian, 2021, JCSR) uses these frames to validate a proposed single-increment predictor-corrector (SIPC) method for second-order elastic analysis against a more exact iterative work-control (WC) method and a first-order linear analysis (LA).
...
**Figure 1** (to be rendered as a proper figure before submission) summarizes the reproduction process as actually followed, rather than as a single re-run: published data -> independent reconstruction -> initial mismatch against the published benchmark -> methodology audit of the companion paper -> identification of the undocumented 0.8EI requirement -> correction -> cross-tool validation. This sequence, not any single tool's output, is the paper's primary contribution.
...
|---|---|---|---|---|
| FRAME3DD 20140514+ (source build) | frame element with geometric stiffness | Newton-Raphson, full | built-in large-displacement N-R | A, Izz, Iyy from original paper |
| OpenSeesPy | elasticBeamColumn + geometric transformation | Newton, incremental | PDelta (Frame 1, 4); Corotational (Frame 4, cross-checked) | A, I from original paper |
| suanPan 4.1.1 | EB21 (linear) / B21 (corotational) | corotational (B21 only) | Corotational | EB21: A, I from paper. B21: built-in AISC US2D W14X68 / W18X35 database |
| CalculiX 2.21 (source build) | U1 user element, SECTION=GENERAL | not supported by this element in v2.21 | n/a (linear only) | A, I11, I22 from original paper |
...
We observed the same underlying reproduction issue in two further benchmark frames from the same released dataset, chosen for structural dissimilarity from Frame 1: a single-story, multi-bay leaning-column frame with a markedly lower critical buckling ratio (in which an even less obvious, member-selective variant of the same connectivity-table trap is present), and an asymmetric two-story, two-bay frame. In both cases, the published linear (LA) benchmark is reproduced to within 0.01% once a uniform 0.8E stiffness is applied to every member, regardless of what the released connectivity table's per-element material assignment literally indicates. For the two-story frame we additionally tested, and ruled out, the two most plausible alternative explanations available in the same released material table: the unreduced nominal stiffness (20% high) and the GMNIA-associated 0.9E reduction also present in every frame's material list (11% high) -- only the 0.8E DAM value matches. This suggests the methodology presented here, and the specific 0.8E correction, are not unique to Frame 1 and may generalize across the released benchmark set. A systematic investigation of the remaining benchmark frames -- including a search for any frame that does *not* require this correction, which would sharpen the finding from "always reduce" into a rule for *when* reduction is needed -- is left for future work.

## Data and code availability

Original data: Ziemian, C.W., Ziemian, R.D. (2021). Benchmark Frames for Structural Analysis. Mendeley Data, v1. https://doi.org/10.17632/39sjhchwtx.1 (CC BY 4.0).

Replication code: to be published to a public git repository (FRAME3DD input deck, OpenSeesPy script, suanPan .supan scripts for EB21 and B21, CalculiX .inp deck with the U1 *USER ELEMENT beam section). All tools used are open source with no proprietary dependencies.

## References

- C.W. Ziemian, R.D. Ziemian (2021). Steel benchmark frames for structural analysis and validation studies. Data in Brief, 39, 107564.
- C.W. Ziemian, R.D. Ziemian (2021). Efficient geometric nonlinear elastic analysis for design of steel structures: Benchmark studies. Journal of Constructional Steel Research, 186, 106870.
- AISC 360, Direct Analysis Method (Chapter C / Appendix 7).
- H.P. Gavin. FRAME3DD: Static and dynamic structural analysis of 2D and 3D frames.
- OpenSeesPy documentation.
- T. Chang. suanPan: an open source FEM framework.
- G. Dhondt. CalculiX CrunchiX User's Manual.
